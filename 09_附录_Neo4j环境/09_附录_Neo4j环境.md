...
在本书中，你将通过基于 Neo4j 的实战示例来学习图论与图算法。我（奥斯卡）之所以选择 Neo4j，是因为我有五年以上使用它构建和分析图的经验。

Neo4j 是一款原生图数据库，从底层开始就是为存储、查询和操作图数据而设计的。它由 Java 实现，其他语言编写的软件可以通过 Cypher 查询语言，经由事务型 HTTP 端点或二进制 Bolt 协议来访问它。在 Neo4j 中，数据以节点（node）和关系（relationship）的形式存储，二者都是数据库中的一等公民。节点表示实体，例如人或企业；关系表示这些实体之间的连接。节点和关系都可以带有属性（property），也就是为节点和关系提供附加信息的键值对。

Neo4j 在设计上具备高度可扩展性。它使用灵活的索引系统来高效地查询和操作数据，并支持满足原子性、一致性、隔离性和持久性（ACID）的事务，以确保数据一致性。它还内置了一门名为 Cypher 的查询语言，专为表达清晰、易于使用地查询和操作图数据而设计。

使用 Neo4j 的另一个好处是，它提供了两个你将会用到的实用插件：

- APOC（Awesome Procedures on Cypher）插件——一个面向 Neo4j 的过程、函数与插件库，提供了广泛的能力，包括数据导入导出、数据转换与操作、日期-时间-区间处理、地理空间处理、文本处理等。
- 图数据科学（GDS，Graph Data Science）插件——一组面向 Neo4j 的图算法与过程，使用户能够对图数据执行高级分析。GDS 为最短路径、PageRank、社区检测等常见图算法提供了高效的并行实现。此外，该插件还包含节点嵌入算法，以及支持节点分类和链接预测的机器学习工作流。

## 1. Cypher 查询语言

Cypher 是一门面向图数据库的声明式查询语言，用于检索和操作存储在图数据库中的数据。Cypher 查询采用简洁、易读的语法编写。下方清单是一个简单的 Cypher 查询示例，它借助 ASCII 艺术风格的图形化写法，直观地描绘出所要查询的关系。
...
openCypher 计划是 Neo4j 与其他若干组织之间的一项合作，旨在推动将 Cypher 查询语言确立为处理图数据的标准。该计划的目标，是打造一门通用语言，使其能够查询任意图数据库，而不受底层技术的限制。为实现这一目标，openCypher 计划以开源许可的方式公开 Cypher 语言规范及相关资源，并鼓励各类组织开发自己的 Cypher 实现。迄今为止，Cypher 查询语言已被 Amazon、AgensGraph、Katana Graph、Memgraph、RedisGraph 和 SAP HANA 等采用（openCypher Implementers Group，无日期）。

此外，还有一个官方的 ISO 项目，致力于提出一门统一的图查询语言（GQL），用于与图数据库交互（GQL Standards Committee，无日期）。GQL 旨在以 SQL 为基础，并整合来自现有图查询语言（包括 Cypher）中经过验证的思想。因此，学习 Cypher 是与图数据库打交道的绝佳起点：它不仅已经与众多图数据库集成，未来还将成为官方 ISO 图查询语言的一部分。如需了解更多信息，可参阅面向 GQL 的图模式匹配提案（Deutsch 等，2022）。

## 2. Neo4j 安装

搭建 Neo4j 环境有几种不同的方式可供选择：
...
Neo4j Desktop 是一款本地的 Neo4j 图数据库管理应用。它让你只需点击几下，就能创建数据库实例并安装官方插件。如果你决定使用 Neo4j Desktop，请按以下步骤成功启动一个已安装 APOC 与 GDS 插件的 Neo4j 数据库实例：

1. 从官方网站下载 Neo4j Desktop 应用（https://neo4j.com/download；见图 A.1）。

    ![图 A.1 下载 Neo4j Desktop](images/09_附录_Neo4j环境/figure_A_1.png)

    _图 A.1 下载 Neo4j Desktop。_

2. 在你的计算机上安装 Neo4j Desktop 应用，然后打开它。
3. 完成注册步骤。你可以输入下载应用时分配到的软件密钥，也可以点击“Register Later”（稍后注册）跳过此步骤（见图 A.2）。

    ![图 A.2 填写个人信息或跳过注册](images/09_附录_Neo4j环境/figure_A_2.png)

    _图 A.2 填写个人信息，或跳过注册步骤。_

4. 首次运行 Neo4j Desktop 时，会自动启动 Movies 数据库管理系统（DBMS）。如果它正在运行，请先停止它（见图 A.3）。

    ![图 A.3 停止默认的 Movie DBMS](images/09_附录_Neo4j环境/figure_A_3.png)

    _图 A.3 停止默认的 Movie DBMS 数据库。_

5. 新增一个本地 DBMS（见图 A.4）。

    ![图 A.4 添加本地 DBMS](images/09_附录_Neo4j环境/figure_A_4.png)

    _图 A.4 添加一个本地 DBMS。_

6. 为 DBMS 名称和密码任意填入取值，并确保选择 5.9.0 或更高版本（见图 A.5）。

    ![图 A.5 设置 DBMS 密码与版本](images/09_附录_Neo4j环境/figure_A_5.png)

    _图 A.5 设置 DBMS 密码与版本。_

7. 安装 APOC 与 GDS 插件：选中该 DBMS，右侧会打开包含 Details、Plugins 和 Upgrade 三个标签页的面板。切换到 Plugins 标签页，然后安装 APOC 与 GDS 插件（见图 A.6）。

    ![图 A.6 安装 APOC 与 GDS 插件](images/09_附录_Neo4j环境/figure_A_6.png)

    _图 A.6 安装 APOC 与 GDS 插件。_

8. 启动数据库（见图 A.7）。

    ![图 A.7 启动数据库](images/09_附录_Neo4j环境/figure_A_7.png)

    _图 A.7 启动数据库。_

9. 打开 Neo4j Browser（见图 A.8）。

    ![图 A.8 打开 Neo4j Browser](images/09_附录_Neo4j环境/figure_A_8.png)

    _图 A.8 打开 Neo4j Browser。_

10. 在 Cypher 编辑器中输入查询即可执行 Cypher。对于较长的 Cypher 语句，你可以使用全屏编辑器选项（见图 A.9）。

    ![图 A.9 Neo4j Browser 中的 Cypher 查询编辑器](images/09_附录_Neo4j环境/figure_A_9.png)

    _图 A.9 Neo4j Browser 中的 Cypher 查询编辑器。_

### 2.2. 使用 Docker 安装 Neo4j

如果你选择通过 Docker 安装 Neo4j，需要在命令行中运行下方清单中的命令。
...
这条命令会在后台启动一个容器化的 Neo4j。通过定义 `NEO4J_PLUGINS` 环境变量，APOC 与 GDS 插件会被自动安装。挂载数据卷以持久化数据库文件是一种良好实践。数据库的用户名和密码则通过 `NEO4J_AUTH` 变量指定。

在执行清单 A.2 中的命令后，在浏览器中访问 http://localhost:7474，输入通过 `NEO4J_AUTH` 变量指定的密码。示例中的密码为 `password`。

### 2.3. Neo4j Aura

Neo4j Aura 是 Neo4j 数据库的一个云端托管实例。除了第 7 章之外，你可以在其余所有章节中使用它——第 7 章需要 GDS 库。遗憾的是，免费版并不提供 GDS 库。如果你希望使用云端托管的 Neo4j Aura 来跟随本书示例，就需要使用支持 GDS 算法的 AuraDS 版本。更多信息可参见 Neo4j 官方网站：https://neo4j.com/product/auradb/。

## 3. Neo4j Browser 配置

Neo4j Browser 有一个对新手友好的特性：它会可视化结果节点之间的所有关系，即便这些关系并不属于查询结果的一部分。为避免混淆，请取消勾选“Connect Result Nodes”（连接结果节点）选项，如图 A.10 所示。

![图 A.10 取消勾选 Connect Result Nodes](images/09_附录_Neo4j环境/figure_A_10.png)

_图 A.10 在 Neo4j Browser 中取消勾选 Connect Result Nodes。_

## 4. 电影数据集

在部分章节中，我们使用了电影数据集（Movies dataset）。这是一个体量小、易于加载的示例数据集。下面，我们将介绍如何把电影数据集加载到你的 Neo4j 实例中。

### 4.1. 通过 Neo4j Query Guide 加载

当你处于 Neo4j Query 或 Neo4j Browser 中时，可以通过侧边栏 guides 中的“Movie Graph Guide”来加载电影数据集，或者在 Neo4j Browser 中执行 `:play movies`。

### 4.2. 试用在线版本

该数据集还提供了一个只读的在线版本，地址为 https://demo.neo4jlabs.com:7473/browser/（也可以通过 Bolt 访问：https://demo.neo4jlabs.com:7687）。数据库名称、用户名和密码均为“movies”。

### 4.3. 通过 Cypher 加载

如果你想直接使用 Cypher 加载电影数据集，请使用下方清单中的查询。

_清单 A.3 通过 Cypher 加载电影数据集_
...
MERGE (JessicaThompson)-[:REVIEWED {
  summary:'You had me at Jerry', rating:92
}]->(JerryMaguire);
```
